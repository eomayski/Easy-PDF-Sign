...
- **Credit debit happens at download time, not at sign time** — 1 credit is debited per signed document, at download-token issuance (`POST /download/request`). Re-downloads with the same token are free (retry after an interrupted stream). Retention: unsigned documents are deleted as soon as the user leaves the flow (`DELETE /api/jobs/:id`, with a 60-min TTL as backstop), signed ones 15 min after signing — extended to the download-token TTL once a credit has been spent, so a paid file can't vanish.
